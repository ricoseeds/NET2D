    ...
    for (int x = 0; x < SCREEN_WIDTH; x++)
    {
        for (int y = 0; y < SCREEN_HEIGHT; y++)
        {
            ...
            if ( SCREEN_WIDTH > SCREEN_HEIGHT ) {
                xamnt = (( x + 0.5) / SCREEN_WIDTH) * aspect_ratio - (((SCREEN_WIDTH - SCREEN_HEIGHT) / (double) SCREEN_HEIGHT) / 2);
                yamnt = ((SCREEN_HEIGHT - y) + 0.5) / SCREEN_HEIGHT;
            } else if (SCREEN_HEIGHT > SCREEN_WIDTH){
                xamnt = (x + 0.5) / SCREEN_WIDTH;
                yamnt = (((SCREEN_HEIGHT - y) + 0.5 ) / SCREEN_HEIGHT)/aspect_ratio - (((SCREEN_HEIGHT - SCREEN_WIDTH)/(double) SCREEN_WIDTH ) / 2);
            } else {
                xamnt = (x + 0.5) / SCREEN_WIDTH;
                yamnt = ((SCREEN_HEIGHT - y ) + 0.5)/ SCREEN_HEIGHT;
            }
